 ...
 * @Last Modified time: 2025-05-05 01:32:42
 */
...
export class MobilePhoneController {
  ...
  @tag('MobilePhone')
  @summary('LinqToJS')
  @description('LinqToJS')
  @parameter('_id', joi.string().description('id'), ENUM_PARAM_IN.query)
  async LinqToJS(req: Request, res: Response, next: NextFunction) {
    const persons = [
      { ID: 0, Age: 30, Name: 'A' },
      { ID: 1, Age: 25, Name: 'B' },
      { ID: 2, Age: 18, Name: 'C' },
    ...
    const result = new Linq<any>(persons, 'zh-CN')
  ...
  async LinqToTS(req: Request, res: Response, next: NextFunction) {
    ...
    const orderByID = new TLinq<any>(persons).orderBy(value => value.ID).toArray();
    const thenByAge = new TLinq<any>(persons)
      .orderBy(value => value.ID)
      .thenBy(value => value.Age)
      .toArray();
    const thenByName = new TLinq<any>(persons)
      .orderBy(value => value.ID)
      .thenBy(value => value.Age)
      .thenByDescending(value => value.Name)
      .toArray();
